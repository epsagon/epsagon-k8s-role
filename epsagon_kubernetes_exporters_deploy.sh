...
function apply_celery_exporter {
    ...
    echo "URL:"
    read broker_url
    cat external-metrics-exporters/celery-exporter.yaml | sed -e "s|\${CELERY_BROKER_URL}|${broker_url}|g" > metrics_exporter.yaml
    install_exporter $CONTEXT $CONFIG "metrics_exporter.yaml"
}

function apply_node_exporter {
    CONTEXT=$1
    CONFIG=$2
    cp external-metrics-exporters/node-exporter.yaml ./metrics_exporter.yaml
    install_exporter $CONTEXT $CONFIG "metrics_exporter.yaml"
}
