...
function test_connection {
    ...
    RESULT="curl -X POST https://api.epsagon.com/containers/k8s/test_cluster_connection -d $DATA -H 'Content-Type: application/json'" | grep -o -E "\"reason\":\".+\"" | awk -F\: '{print $2}')
    ...
    # }, "status": 200}
    if $RESULT; then
        echo "Could not connect to server ${SERVER}, error message: ${RESULT}"
        return 1
    fi
    else
